...
def get_token_ids_from_config(config) -> Dict[str, int]:
    ...
    if text_config is not None:
        ...
        token_ids["default_mask_token_id"] = getattr(
...
def sample_tokens_batched(
    ...
    **generate_kwargs,
    ...
    """Batched form of ``sample_tokens`` for the multi-row decoder.

    Identical pipeline (repetition penalty -> temperature/top-p/top-k -> softmax ->
    argmax/sample) but operates on a real batch ``[B, seq_len, V]`` and returns
    ``box_avg`` as a per-row LIST instead of ``torch.stack``-ing it -- each row's
    decoded box/ref is ragged (different length) so it cannot be stacked. This is
    numerically identical to calling ``sample_tokens`` once per row: the logits
    pipeline is independent per row, and ``generated`` may be right-padded with -1,
    which ``apply_repetition_penalty`` filters as out-of-vocab so the pad never
    enters any row's penalty mask. ``box_avg`` is ``None`` for single-step (AR)
    windows, matching ``sample_tokens``.
    """
    batch_size, seq_len, vocab_size = logits.shape
...
def sample_tokens_ar(
    ...
    **generate_kwargs,
    ...
    assert (
        seq_len == 1
    ), "sample_tokens_ar only supports single-step AR sampling (seq_len == 1)"
...
def is_valid_box_frame(
    ...
    end_target_ids = torch.tensor(
        [box_end_token_id, null_token_id, im_end_token_id], device=probs.device
    )
...
def decode_bbox_avg(
    ...
    box_type = is_valid_box_frame(
        probs, token_ids, start_thresh=start_thresh, end_thresh=end_thresh, topk=keep_k
    )
    ...
    if generation_mode == "hybrid":
        ...
        valid_ids_for_max = torch.where(
            mask, pos_ids, torch.tensor(SMALL_NUM, device=device)
        )
        valid_ids_for_min = torch.where(
            mask, pos_ids, torch.tensor(LARGE_NUM, device=device)
        )
        ...
        final_coords = torch.where(
            is_abnormal, torch.tensor(0, device=pos_ids.device), first_valid_ids
        )
    elif generation_mode == "fast":
        final_coords = first_valid_ids

    start_t = torch.tensor(
        [box_start_token_id], dtype=final_coords.dtype, device=device
    )
...
def decode_ref(
    ...
    start_t = torch.tensor(
        [ref_start_token_id], dtype=final_text_ids.dtype, device=device
    )
