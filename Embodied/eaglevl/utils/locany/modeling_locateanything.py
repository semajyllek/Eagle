...
from .generate_utils import (get_token_ids_from_config, handle_pattern,
                             sample_tokens)
...
class LocateAnythingForConditionalGeneration(
    LocateAnythingPreTrainedModel, GenerationMixin
):
    config_class = LocateAnythingConfig

    def __init__(
        self, config: LocateAnythingConfig, vision_model=None, language_model=None
    ):
        ...
        if vision_model is not None:
            ...
            if config.vision_config.model_type == "moonvit":
                vision_attn_impl = (
                    getattr(config.vision_config, "_attn_implementation", None)
        ...
        if language_model is not None:
            ...
            if config.text_config.architectures[0] == "Qwen2ForCausalLM":
                ...
                raise ValueError(
                    f"Unsupported language model architecture: {config.text_config.architectures[0]}. Only Qwen2ForCausalLM and Qwen3ForCausalLM are supported."
                )
        ...
        arch = (
            ...
            if hasattr(config.text_config, "architectures")
            and config.text_config.architectures
    ...
    def forward(
        ...
        if has_images:
            ...
            input_embeds[selected] = (
                input_embeds[selected] * 0.0 + vit_embeds[: selected.sum()]
            )
        else:
            if vit_embeds:
                vit_embeds = torch.cat(vit_embeds, dim=0)
    ...
    def extract_feature(self, pixel_values, image_grid_hws):
        vit_embeds = self.vision_model(
            pixel_values=pixel_values, grid_hws=image_grid_hws
        )
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        generate_kwargs.pop("early_eject", None)
        ...
        assert generate_kwargs.get(
            "use_cache", False
        ), "Only use_cache=True is supported."
        ...
        total_gen_length = min(
            tokenizer.model_max_length,
            seq_len + generate_kwargs.get("max_new_tokens", 2048),
        )
        iter_round = 0
        ...
        full_position_ids = torch.arange(
            0, max_possible_len, device=generated.device
        ).unsqueeze(0)
        ...
        def _prepare_inputs_in_mtp(generated):
            ...
            position_ids = full_position_ids[
                :, start_idx : generated_with_mask.size(1)
            ].clone()
        ...
        while generated.size(1) < total_gen_length:
            ...
            if verbose:
                sampling_history.append(
                    (
                        "ar" if "ar" in out_type else "mtp",
                        tokenizer.decode(out_token, skip_special_tokens=False),
                    )
                )
